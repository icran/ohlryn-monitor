...
| | vector-backtester (엔진) | ohlryn-monitor (관측/알림) |
|---|---|---|
...
**경계 규칙**: `vector_backtester`를 import하면 → 엔진 레포. 봇 HTTP(`/api/v1`)나 거래소 API로만 대화하면 → 여기(ohlryn-monitor).

## 설계 원칙 — 순수 로직 + I/O 분리 (ports & adapters)

- **순수 로직** (`ohlryn_monitor/signals.py` 등): 네트워크·파일 없이 입력→출력. 단위 테스트 대상.
- **I/O 어댑터** (`ohlryn_monitor/prices.py`, `notify.py`, `state.py`): 거래소 price fetch, 텔레그램 전송, JSON state.
- **오케스트레이션** (`ohlryn_monitor/alerters/*`): 순수 로직 + 어댑터를 조립한 `main()`.
