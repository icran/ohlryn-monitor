...
# 윈도 내 사망 N회 → 차단.
# ⚠ 이 백스톱은 "수동 재시작"과 "진짜 크래시루프"를 구분하지 못한다 — config 를 고치며
#    봇을 여러 번 내렸다 올리면 그것도 사망으로 세어 정상 작업 중에 차단이 걸린다.
#    2026-08-24 에 3 → 5 로 올렸다 (설정 수정 한 회차에 4회까지 재시작하는 일이 있다).
#    진짜 고장에 대한 보호는 위 BOOT_STREAK_N(=2) 이 담당한다 — 기동 120초 내 사망이
#    연속되면 이 값과 무관하게 차단되므로, 여기를 완화해도 부팅 불가 config 는 못 지나간다.
WINDOW_N = 5
