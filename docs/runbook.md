...
### watchdog crash-loop 차단

| 알림 | 의미 | 대응 |
|------|------|------|
| `🚨 crash loop 차단: <봇>` | 부팅 내 사망 연속 2회 또는 30분 내 3회 — **구조적 문제로 판단, 소생 중단됨**. 알림에 사유·동일 에러 여부·마지막 로그 30줄 포함 | 로그로 원인 파악·수정 → `rm ~/wd_state/<봇>.crashloop` → 다음 주기(≤5분) 자동 재기동 |

### schedule_watch (`[my-schedule]` 프리픽스)

| 알림 | 의미 | 대응 |
|------|------|------|
| `⚠️ WARNING 스케줄 만료 임박: <이름> — D-N` | 파라미터 스케줄(CSV)이 N일 후 만료 | 만료 전에 wfa.update로 스케줄 연장 (연장하면 경고 자동 해제) |
| `🚨 CRITICAL 스케줄 만료: <이름>` | **만료됨 — 이후 봇 재시작 시 파라미터가 기본값으로 회귀(방어 필터 OFF)** | 즉시 스케줄 연장(임시: carry_forward 행 추가) 후 봇 재시작. 2026-07-31 사고 참조 |
| `🚨 CRITICAL 스케줄 감시 실패` | CSV 읽기 실패 — 경로/포맷 문제 | config의 csv 경로 확인 |
